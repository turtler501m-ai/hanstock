...
### 1.6 [스케줄] GitHub Actions가 자동 실행이 아니었고 mode가 무시됨

- `.github/workflows/auto_trade.yml`에 `schedule:` cron 트리거가 없어
  `workflow_dispatch`(수동)로만 동작했다. 이름은 "자동 매매"였다.
...
- `schedule:` cron 추가: `'0 0-6 * * 1-5'`(평일 KST 09–15시, UTC 기준).
...
| GitHub Actions | 수동 실행만, mode 무시 | cron 자동 실행 + mode 반영 |
