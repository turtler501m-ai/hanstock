...
- **`min_ai_confidence`를 실제 게이트로 적용**(후속 보강): `predict()`에서
  `probability < min_confidence`이면 더 이상 final_score에 블렌딩하지 않고
  **룰 점수로 폴백**한다(`score_weight=0`, `model_status="low_confidence"`,
  `fallback_reason` 기록). 이전에는 신뢰도 미달이어도 라벨만 찍고 저신뢰 AI 점수가
  그대로 섞여, 후보를 승격/강등시킬 수 있었다. 폴백으로 바뀌면서 저신뢰 AI는
  점수를 올리지도 내리지도 못한다.
...
| `min_ai_confidence` | 라벨만 표시(저신뢰 점수 그대로 블렌딩) | 미달 시 룰 점수로 폴백(실제 게이트) |
